...
parainit()
local screen, reward, terminal = NewGame()
...
while step < opt.steps do
    ...
    if not terminal then
        screen, reward, terminal = EvasionGame(game_actions[action_index])
    else
        if opt.random_starts > 0 then
            screen, reward, terminal = NewGame()
            screen, reward, terminal = NextRandGame()
        else
            screen, reward, terminal = NewGame()
        end
    end
